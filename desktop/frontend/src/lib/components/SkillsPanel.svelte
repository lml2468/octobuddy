<script lang="ts">
  ...
  async function deleteFile(rel: string) {
    ...
    if (!(await ask(`删除文件 ${rel}?`, { confirmLabel: "删除", danger: true }))) return;
    try {
  ...
  async function removeBotSkill(s: SkillInfo) {
    ...
    if (!(await ask(`${verb}「${s.name}」?`, { confirmLabel: verb, danger: true }))) return;
    try {
...
<style>
  ...
  .row:focus-visible, .rowmain:focus-visible, .add:focus-visible, .primary:focus-visible, .fname:focus-visible, .del:focus-visible, .inst:focus-visible { outline: none; box-shadow: 0 0 0 3px color-mix(in srgb, var(--accent) 30%, transparent); }
